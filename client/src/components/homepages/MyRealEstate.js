...
const MyRealEstate = () =>{
    ...
    const realEstate = useSelector((state)=>state.realestate.filter( r => r.user_id===userLogin._id ));
    ...
    const realEstateFollow = useSelector((state)=>state.realestate.filter(r=>statusLike.includes(r._id)));
    ...
    return(
        ...
        <Grid container spacing={5} className={classes.cardContent}>
                ...
                <BsStar/> bất động sản của bạn đã đăng
                </Grid>
            ...
            <Grid item xs={12} className={classes.titleChiMuc}>
                <BsStar/> Bất động sản đang theo dõi
            </Grid>
            {
                realEstateFollow.sort((a, b) => new Date(b.post_date) - new Date(a.post_date))
